...
DEFAULT_DATASET = ROOT / "training_data" / "sql_database"
DEFAULT_BASE = ROOT / "artifacts" / "checkpoints" / "fodci-python-backend-v1.pt"
DEFAULT_CHECKPOINT = ROOT / "artifacts" / "checkpoints" / "fodci-sql-database-v1.pt"
DEFAULT_REPORT = ROOT / "artifacts" / "evaluation" / "phase135_sql_database_training.json"
DEFAULT_MARKDOWN = ROOT / "docs" / "experiments" / "phase135_sql_database_training.md"
SEED = 2026
BASE_VERSION = "fodci-python-backend-v1"
MODEL_VERSION = "fodci-sql-database-v1"
DATASET_VERSION = "sql-database-specialist-v1"
...
def evaluate(model: FodciModel, examples: list[Any], dataset_path: Path) -> dict[str, Any]:
    config = EvaluationConfig(batch_size=2, device="cpu", seed=SEED, dataset_path=dataset_path, dataset_split="sql-database-validation", checkpoint_dir=DEFAULT_CHECKPOINT.parent, model_version=MODEL_VERSION)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Phase 13.5 — SQL & Database Reasoning Training
...
| Gate | Result |
|---|---|
| Base checkpoint version is Phase 13.4 | `{gates['base_checkpoint_compatible']}` |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune the Fodci Stage 13.4 checkpoint on SQL and database reasoning patterns.")
    ...
    trainer = FodciTrainer(training_model, source(train_examples), source(validation_examples), training_config, model_version=MODEL_VERSION, checkpoint_run_metadata={"phase": "13.5", "base_checkpoint": str(base_checkpoint), "dataset_version": DATASET_VERSION})
    ...
    if not gates["all_passed"]:
        raise RuntimeError(f"Phase 13.5 validation failed: {gates}")
    report = {
        "format": "fodci.phase135_sql_database_training",
        "schema_version": "1.0",
        "phase": "13.5",
    ...
    print(json.dumps({"phase": "13.5", "checkpoint": str(checkpoint), "base_loss": base_eval["loss"], "trained_loss": trained_eval["loss"], "global_step": training_result.global_step, "validation_gates": gates, "report": str(args.report)}, ensure_ascii=False, indent=2))
