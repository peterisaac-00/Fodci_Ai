...
DEFAULT_DATASET = ROOT / "training_data" / "debugging"
DEFAULT_BASE = ROOT / "artifacts" / "checkpoints" / "fodci-rest-api-v1.pt"
DEFAULT_CHECKPOINT = ROOT / "artifacts" / "checkpoints" / "fodci-debugging-v1.pt"
DEFAULT_REPORT = ROOT / "artifacts" / "evaluation" / "phase137_debugging_training.json"
DEFAULT_MARKDOWN = ROOT / "docs" / "experiments" / "phase137_debugging_training.md"
SEED = 2026
BASE_VERSION = "fodci-rest-api-v1"
MODEL_VERSION = "fodci-debugging-v1"
DATASET_VERSION = "debugging-root-cause-specialist-v1"
...
def evaluate(model: FodciModel, examples: list[Any], dataset_path: Path) -> dict[str, Any]:
    config = EvaluationConfig(batch_size=2, device="cpu", seed=SEED, dataset_path=dataset_path, dataset_split="debugging-validation", checkpoint_dir=DEFAULT_CHECKPOINT.parent, model_version=MODEL_VERSION)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Phase 13.7 — Debugging & Root Cause Analysis Training
...
| Gate | Result |
|---|---|
| Base checkpoint version is Phase 13.6 | `{gates['base_checkpoint_compatible']}` |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune the Fodci Stage 13.6 checkpoint on debugging and root-cause analysis patterns.")
    ...
    trainer = FodciTrainer(training_model, source(train_examples), source(validation_examples), training_config, model_version=MODEL_VERSION, checkpoint_run_metadata={"phase": "13.7", "base_checkpoint": str(base_checkpoint), "dataset_version": DATASET_VERSION})
    ...
    if not gates["all_passed"]:
        raise RuntimeError(f"Phase 13.7 validation failed: {gates}")
    report = {
        "format": "fodci.phase137_debugging_training",
        "schema_version": "1.0",
        "phase": "13.7",
    ...
    print(json.dumps({"phase": "13.7", "checkpoint": str(checkpoint), "base_loss": base_eval["loss"], "trained_loss": trained_eval["loss"], "global_step": training_result.global_step, "validation_gates": gates, "report": str(args.report)}, ensure_ascii=False, indent=2))
