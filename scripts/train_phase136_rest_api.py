...
DEFAULT_DATASET = ROOT / "training_data" / "rest_api"
DEFAULT_BASE = ROOT / "artifacts" / "checkpoints" / "fodci-sql-database-v1.pt"
DEFAULT_CHECKPOINT = ROOT / "artifacts" / "checkpoints" / "fodci-rest-api-v1.pt"
DEFAULT_REPORT = ROOT / "artifacts" / "evaluation" / "phase136_rest_api_training.json"
DEFAULT_MARKDOWN = ROOT / "docs" / "experiments" / "phase136_rest_api_training.md"
SEED = 2026
BASE_VERSION = "fodci-sql-database-v1"
MODEL_VERSION = "fodci-rest-api-v1"
DATASET_VERSION = "rest-api-specialist-v1"
...
def evaluate(model: FodciModel, examples: list[Any], dataset_path: Path) -> dict[str, Any]:
    config = EvaluationConfig(batch_size=2, device="cpu", seed=SEED, dataset_path=dataset_path, dataset_split="rest-api-validation", checkpoint_dir=DEFAULT_CHECKPOINT.parent, model_version=MODEL_VERSION)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Phase 13.6 — RESTful API Design & Implementation Training
...
| Gate | Result |
|---|---|
| Base checkpoint version is Phase 13.5 | `{gates['base_checkpoint_compatible']}` |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune the Fodci Stage 13.5 checkpoint on RESTful API design and implementation patterns.")
    ...
    trainer = FodciTrainer(training_model, source(train_examples), source(validation_examples), training_config, model_version=MODEL_VERSION, checkpoint_run_metadata={"phase": "13.6", "base_checkpoint": str(base_checkpoint), "dataset_version": DATASET_VERSION})
    ...
    if not gates["all_passed"]:
        raise RuntimeError(f"Phase 13.6 validation failed: {gates}")
    report = {
        "format": "fodci.phase136_rest_api_training",
        "schema_version": "1.0",
        "phase": "13.6",
    ...
    print(json.dumps({"phase": "13.6", "checkpoint": str(checkpoint), "base_loss": base_eval["loss"], "trained_loss": trained_eval["loss"], "global_step": training_result.global_step, "validation_gates": gates, "report": str(args.report)}, ensure_ascii=False, indent=2))
