...
DEFAULT_BASE = ROOT / "artifacts" / "checkpoints" / "fodci-security-auth-v1.pt"
DEFAULT_CHECKPOINT = ROOT / "artifacts" / "checkpoints" / "fodci-testing-qa-v1.pt"
DEFAULT_REPORT = ROOT / "artifacts" / "evaluation" / "phase139_testing_qa_training.json"
DEFAULT_MARKDOWN = ROOT / "docs" / "experiments" / "phase139_testing_qa_training.md"
SEED = 2026
BASE_VERSION = "fodci-security-auth-v1"
...
def evaluate(model: FodciModel, examples: list[Any], dataset_path: Path) -> dict[str, Any]:
    config = EvaluationConfig(batch_size=2, device="cpu", seed=SEED, dataset_path=dataset_path, dataset_split="testing-qa-validation", checkpoint_dir=DEFAULT_CHECKPOINT.parent, model_version=MODEL_VERSION)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Phase 13.9 — Testing & Quality Assurance Training
...
| Gate | Result |
|---|---|
| Base checkpoint version is Phase 13.8 | `{gates['base_checkpoint_compatible']}` |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune the Fodci Phase 13.8 checkpoint on testing and quality assurance patterns.")
    ...
    trainer = FodciTrainer(training_model, source(train_examples), source(validation_examples), training_config, model_version=MODEL_VERSION, checkpoint_run_metadata={"phase": "13.9", "base_checkpoint": str(base_checkpoint), "dataset_version": DATASET_VERSION})
    ...
    if not gates["all_passed"]:
        raise RuntimeError(f"Phase 13.9 validation failed: {gates}")
    report = {
        "format": "fodci.phase139_testing_qa_training",
        "schema_version": "1.0",
        "phase": "13.9",
    ...
    print(json.dumps({"phase": "13.9", "checkpoint": str(checkpoint), "base_loss": base_eval["loss"], "trained_loss": trained_eval["loss"], "global_step": training_result.global_step, "validation_gates": gates, "report": str(args.report)}, ensure_ascii=False, indent=2))
