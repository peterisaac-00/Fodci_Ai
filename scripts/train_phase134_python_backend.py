...
DEFAULT_DATASET = ROOT / "training_data" / "python_backend"
DEFAULT_BASE = ROOT / "artifacts" / "checkpoints" / "fodci-stage1-v1.pt"
DEFAULT_CHECKPOINT = ROOT / "artifacts" / "checkpoints" / "fodci-python-backend-v1.pt"
DEFAULT_REPORT = ROOT / "artifacts" / "evaluation" / "phase134_python_backend_training.json"
DEFAULT_MARKDOWN = ROOT / "docs" / "experiments" / "phase134_python_backend_training.md"
SEED = 2026
BASE_VERSION = "fodci-stage1-v1"
MODEL_VERSION = "fodci-python-backend-v1"
DATASET_VERSION = "python-backend-specialist-v1"
...
def make_pipeline(path: Path, tokenizer: FodciTokenizer) -> InstructionDatasetPipeline:
    return InstructionDatasetPipeline(
        DatasetConfig(path, supported_extensions=frozenset({".txt"}), context_length=128, use_eos_document_boundaries=False),
        tokenizer,
    )
...
def evaluate(model: FodciModel, examples: list[Any], dataset_path: Path) -> dict[str, Any]:
    config = EvaluationConfig(batch_size=2, device="cpu", seed=SEED, dataset_path=dataset_path, dataset_split="python-backend-validation", checkpoint_dir=DEFAULT_CHECKPOINT.parent, model_version=MODEL_VERSION)
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    return f"""# Phase 13.4 — Python for Backend Specialist Training
...
| Gate | Result |
|---|---|
| Base checkpoint version is Stage 13.3 | `{gates['base_checkpoint_compatible']}` |
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune the Fodci Stage 13.3 checkpoint on Python backend patterns.")
    ...
    trainer = FodciTrainer(training_model, source(train_examples), source(validation_examples), training_config, model_version=MODEL_VERSION, checkpoint_run_metadata={"phase": "13.4", "base_checkpoint": str(base_checkpoint), "dataset_version": DATASET_VERSION})
    ...
    if not gates["all_passed"]:
        raise RuntimeError(f"Phase 13.4 validation failed: {gates}")
    report = {
        "format": "fodci.phase134_python_backend_training",
        "schema_version": "1.0",
        "phase": "13.4",
    ...
    print(json.dumps({"phase": "13.4", "checkpoint": str(checkpoint), "base_loss": base_eval["loss"], "trained_loss": trained_eval["loss"], "global_step": training_result.global_step, "validation_gates": gates, "report": str(args.report)}, ensure_ascii=False, indent=2))
