...
Phase 6.5 does not implement Phase 6.6 Error Recovery, retries, self-correction, autonomous debugging, memory, RAG, network, package installation, Git mutation, shell execution, background agents, scheduling, daemon processes, unrestricted command/file modification, or automatic CLI autonomy.
...
| Agent adapter | Keep `ProviderBackedAgent` compatibility and bounded read-only `AgentLoop` orchestration over the default registry; expose Planner, ToolSelector, explicitly constructed `AutonomousToolLoop`, pure stop-condition APIs, and explicit host-configured execution budgets as separate APIs without changing CLI/default AgentLoop behavior or auto-enabling mutation/execution capabilities | Phase 6.6+ recovery systems, automatic debugging/fixing/retries, automatic CLI autonomy, Git mutation, memory, RAG, autonomous/background loops |
