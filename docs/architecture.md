...
Phase 13.8 specializes the Phase 13.7 debugging checkpoint on security patterns required by backend services. The dataset contains 32 training records and 8 held-out validation records balanced across JWT validation, OAuth2 flows, password hashing, and authentication middleware. The records emphasize fail-closed verification, explicit algorithm and audience policies, PKCE and state protection, least privilege, bounded token lifetimes, adaptive password hashing, redaction, tenant isolation, and stable security error contracts.

`scripts/train_phase138_security_auth.py` validates that the input checkpoint is exactly `fodci-debugging-v1`, evaluates it on the security validation partition, continues bounded CPU training, saves `fodci-security-auth-v1.pt`, reloads it into a fresh model, and verifies finite loss, parameter changes, and reload consistency. The implementation does not add runtime authentication features to Fodci and does not claim production security certification; it is a controlled model-specialization experiment.

The held-out benchmark is stored separately at `training_data/security_auth/evaluation/phase_138.jsonl` and is executed by the shared deterministic benchmark runner. Its keyword coverage and non-empty-output results are diagnostic proxies only. Security correctness still requires threat modeling, code review, dependency review, and execution-aware tests against concrete implementations.

## Phase 13.9 — Testing & Quality Assurance Training

Phase 13.9 specializes the Phase 13.8 security checkpoint on the testing discipline required by a backend engineering agent. The dataset contains 32 training records and 8 held-out validation records balanced across unit testing, integration testing, Pytest fixtures and test doubles, and code coverage. The curriculum emphasizes deterministic tests, public contracts, isolated boundaries, reliable cleanup, meaningful assertions, branch-aware coverage review, and risk-based quality decisions.
...
Phase 13.10 adds a bounded CPU-only scaling experiment without changing the default Fodci runtime or any existing checkpoint. The control is the verified `fodci-testing-qa-v1` checkpoint with 11,424,400 parameters. The experimental candidate uses hidden size 448, seven Transformer layers, seven attention heads, a 1,792-unit feed-forward layer, the same 10,000-token vocabulary and 256-token context, and contains 25,985,488 parameters—approximately 2.27 times the control.

`scripts/analyze_phase1310_scaling.py` measures parameter counts, shape compatibility, forward/backward loss, finite gradients, resident memory after backward, short four-step CPU training, and validation loss on the Phase 13.9 testing corpus. The larger candidate starts from random initialization because the existing 11.4M checkpoint is not shape-compatible. It completes a bounded four-step CPU training run, saves `fodci-scaling-25m-experimental-v1.pt`, and passes checkpoint reload validation. Its loss is not a fair capability comparison against the fully trained 11.4M checkpoint, and the experimental artifact is not wired into the normal runtime.
...
Phase 13.11 validates the integration boundary between the trained `fodci-testing-qa-v1` checkpoint, the typed local provider, `AgentOrchestrator`, `AdvancedMemorySystem`, and `AutonomyController`. The workflow loads the real checkpoint on CPU, performs a four-token bounded provider smoke, passes its output as non-authoritative context to a dependency-ordered planner/coder/tester/verifier workflow, persists successful subtask evidence, reloads memory from disk, retrieves the evidence, and executes a second bounded autonomy task.

The integration gates cover checkpoint identity, provider generation, complete shared task state, project-scoped memory persistence and retrieval after restart, completed autonomy lifecycle, budget limits, and preservation of the default runtime. The Phase 13.10 approximately 26M checkpoint remains explicitly experimental and is not activated by this phase. Synergy evidence establishes architectural compatibility and state flow; it does not establish semantic agent capability or production-grade autonomous repair.

## Phase 13.12 — Final Evaluation & Feature Complete

Phase 13.12 performs the final release audit across the complete project lineage. `scripts/run_phase1312_final_evaluation.py` audits every stable specialist checkpoint from `fodci-tiny-v1` through `fodci-testing-qa-v1`, validates all persisted specialist training gates and benchmark report schemas, checks the Phase 13.10 scaling evidence, rechecks Phase 13.11 synergy evidence, performs a bounded CPU runtime smoke with the stable checkpoint, and records the full regression and compileall results.

The release gates distinguish engineering completeness from model capability. The feature-complete release requires valid checkpoint lineage, finite and reload-verified training evidence, structurally valid benchmark reports, valid scaling and synergy evidence, stable runtime loading, full regression evidence, and preservation of the 11.4M default. A zero benchmark pass rate or zero non-empty rate remains visible as an honest capability limitation rather than being converted into a release-gate failure.

The approximately 26M scaling checkpoint remains an experimental artifact and is not activated. The final stable runtime remains CPU-compatible, zero-external-API, and based on the 11,424,400-parameter `fodci-testing-qa-v1` checkpoint. Because this stable checkpoint is approximately 137MB, it is distributed through the `v13.12-stable` GitHub Release asset rather than normal Git history. `Application` prefers the stable checkpoint when present and falls back to `fodci-tiny-v1.pt` for backward compatibility; `scripts/download_phase1312_checkpoint.ps1` downloads and verifies the release asset by SHA-256.

## Phase 13.13 — English Language Foundation & 25M Training

Phase 13.13 introduces an English-only foundation-training path for the decoder-only Fodci model. The corpus is built from four Project Gutenberg books for training and one held-out book for validation, with source hashes and provenance recorded in `training_data/english_foundation/manifest.json`. The matched tokenizer artifact is `tokenizers/fodci-english-v4.json`, a bounded byte-BPE tokenizer with 512 learned merges and the required 10,000-token model vocabulary.

The CPU-only experiment trains both the stable-sized 11,424,400-parameter configuration and an experimental 25,985,488-parameter configuration. Each candidate records checkpoint lineage, finite validation loss, parameter changes, non-empty train/validation splits, checkpoint existence, and checkpoint reload. The measured losses improved under the bounded schedule, but natural-response probes remained repetitive or nonsensical. Structural training success is therefore explicitly separated from human-readable response quality.
