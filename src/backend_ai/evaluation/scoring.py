...
class BenchmarkScorer:
    ...
    def evaluate_task(self, task: EvaluationTask, task_run: BenchmarkTaskRun) -> TaskEvaluation:
        ...
        declared = {criterion.criterion_id: criterion for criterion in task.success_criteria}
        ...
        missing = tuple(item.criterion_id for item in criteria if declared[item.criterion_id].required and item.status in (EvaluationStatus.INSUFFICIENT_EVIDENCE, EvaluationStatus.UNAVAILABLE, EvaluationStatus.BUDGET_EXHAUSTED))
        blockers = tuple(item.explanation for item in criteria if declared[item.criterion_id].required and item.status in (EvaluationStatus.FAILED, EvaluationStatus.BLOCKED, EvaluationStatus.REGRESSION_DETECTED))
        ...
        if blockers or task_run.status is BenchmarkTaskStatus.FAILED or success_status in (EvaluationStatus.FAILED, EvaluationStatus.BLOCKED, EvaluationStatus.REGRESSION_DETECTED):
            status = EvaluationStatus.FAILED
        elif success_status in (EvaluationStatus.INSUFFICIENT_EVIDENCE, EvaluationStatus.UNAVAILABLE, EvaluationStatus.BUDGET_EXHAUSTED) or missing:
            status = EvaluationStatus.INCOMPLETE
        elif success_status is EvaluationStatus.VERIFIED:
    ...
    def _criterion(self, task: EvaluationTask, run: BenchmarkTaskRun, criterion: SuccessCriterion) -> CriterionEvaluation:
        ...
        if ctype in ("TEST_PASS", "REGRESSION_FREE"):
            if ev.test_result is None or not ev.tests_executed:
                return CriterionEvaluation(criterion.criterion_id, EvaluationStatus.UNAVAILABLE, None, 0.0, ("test_result",), "Required test evidence was not executed or is unavailable.", "NONE")
            evidence.append("test_result")
            if "REGRESSION" in ctype and (_has_regression(ev) or final in ("NOT_VERIFIED", "FAILED")):
                return CriterionEvaluation(criterion.criterion_id, EvaluationStatus.REGRESSION_DETECTED, False, 0.0, tuple(evidence + ["final_verification"]), "Regression or final verification failure detected.", "STRONG")
        ...
        if not ev.evidence_complete or (ev.final_verification_evidence is None and ev.completion_evidence is None):
            return CriterionEvaluation(criterion.criterion_id, EvaluationStatus.INSUFFICIENT_EVIDENCE, None, 0.0, ("evidence_complete",), "Evidence is incomplete; criterion cannot be evaluated.", "NONE")
        authoritative = _status_value(ev.final_verification_evidence) or _status_value(ev.completion_evidence)
        satisfied = authoritative in ("VERIFIED", "COMPLETE", "COMPLETED", "PASSED")
        return CriterionEvaluation(criterion.criterion_id, EvaluationStatus.VERIFIED if satisfied else EvaluationStatus.FAILED, satisfied, 1.0 if satisfied else 0.0, ("authoritative_evidence",), "Authoritative evidence supports the criterion." if satisfied else "Authoritative evidence does not support the criterion.", "STRONG")
    ...
    def _task_success(self, task: EvaluationTask, run: BenchmarkTaskRun, criteria: Sequence[CriterionEvaluation]) -> tuple[float, EvaluationStatus, tuple[str, ...]]:
        ...
        if not run.evidence.evidence_complete or run.evidence.final_verification_evidence is None or run.evidence.completion_evidence is None:
            return 0.0, EvaluationStatus.INSUFFICIENT_EVIDENCE, ("final_verification_evidence", "completion_evidence", "evidence_complete")
        ...
        if any(item.status in (EvaluationStatus.FAILED, EvaluationStatus.BLOCKED, EvaluationStatus.REGRESSION_DETECTED) for item in required) or run.evidence.forbidden_changes_detected or _has_regression(run.evidence) or final not in ("VERIFIED", "PASSED") or complete not in ("COMPLETE", "COMPLETED", "PASSED"):
    ...
    def _tests(self, run: BenchmarkTaskRun) -> tuple[float, EvaluationStatus, tuple[str, ...]]:
        ...
        if not run.evidence.tests_executed or run.evidence.test_result is None: return 0.0, EvaluationStatus.UNAVAILABLE, ("test_result",)
        status = _status_value(run.evidence.test_result)
        if _has_regression(run.evidence): return 0.0, EvaluationStatus.REGRESSION_DETECTED, ("regression", "test_result")
    ...
    def _quality(self, run: BenchmarkTaskRun) -> tuple[float, EvaluationStatus, tuple[str, ...]]:
        if not run.evidence.evidence_complete:
            return 0.0, EvaluationStatus.INSUFFICIENT_EVIDENCE, ("evidence_complete",)
    ...
    def _efficiency(self, run: BenchmarkTaskRun, correctness: float) -> tuple[float, EvaluationStatus, tuple[str, ...]]:
        ...
        usage = budget.get("usage") if isinstance(budget.get("usage"), Mapping) else budget
        duration = _number(usage, "duration_seconds", run.evidence.duration_seconds)
        iterations = _number(usage, "iterations", _number(usage, "iteration_count", 0.0))
        calls = _number(usage, "tool_calls_attempted", _number(usage, "tool_calls", _number(usage, "tool_call_count", 0.0)))
        tests = _number(usage, "test_executions", _number(usage, "test_runs", 0.0))
    ...
    def _aggregate_dimension(self, scores: Sequence[TaskScore], name: str) -> ScoreDimension:
        field_name = "test_score" if name == "tests" else f"{name}_score"
        score = sum(getattr(item, field_name) for item in scores) / len(scores)
        weight = getattr(self.weights, "tests" if name == "tests" else name)
        return ScoreDimension(name, score, weight, score * weight, EvaluationStatus.PASS, tuple(item.task_id for item in scores), "Arithmetic mean across all evaluated tasks, including failed and incomplete tasks.")
...
def _has_regression(evidence: Any) -> bool:
    text = " ".join((str(evidence.execution_status), str(evidence.termination_reason), " ".join(evidence.failure_information))).upper()
    status = _status_value(evidence.final_verification_evidence)
    regression_status = str(evidence.final_verification_evidence.get("regression_status", "")).upper() if evidence.final_verification_evidence else ""
