...
> **Current status: Phase 4.6 — Read-only Git status inspection; final Phase 4 sub-phase.**
...
`ToolRegistry.with_git_inspection()` now explicitly exposes both `git_diff` and `git_status`; `ToolRegistry.default()` remains read-only and unchanged, and `AgentLoop` is not modified to invoke Git status. Phase 4.6 is the final sub-phase of Phase 4; Phase 5 and later work are not included.
...
| 4.6 | Read-only Git status inspection (`git_status`); final Phase 4 sub-phase |
...
Phase 4.6 adds only the read-only bounded `git_status` tool, reusable Git adapter extension, immutable status result models, temporary-repository tests, documentation, and explicit Git-inspection registry integration. It does not add Git mutation, commits, staging, reset/restore/checkout/clean/merge/rebase/cherry-pick/push/pull/fetch/clone/init/rm, command or terminal execution, package installation, network access, memory, RAG, embeddings, external APIs, planning, autonomous Git behavior, or Phase 5 functionality.
