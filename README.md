...
> **Current status: Phase 4.5 — Read-only Git diff inspection; Phase 4.5 only.**
...
`ToolRegistry.default()` remains unchanged and does not expose `git_diff`. `ToolRegistry.with_git_inspection()` is an explicit read-only registry containing the Phase 3 tools plus `git_diff`. `AgentLoop` does not automatically receive or invoke Git diff, and Phase 4.5 does not implement `git_status`, Git mutation, command execution, terminal access, network access, memory, RAG, or autonomous behavior.
...
| 4.6+ | Git status and later Phase 4 work |
...
Phase 4.5 adds only the read-only bounded `git_diff` tool, whitelisted Git adapter, immutable result models, temporary-repository tests, documentation, and explicit Git-inspection registry integration. It does not add `git_status`, Git mutation, commits, staging, reset/restore/checkout/clean/push/pull/fetch, command or terminal execution, package installation, network access, memory, RAG, embeddings, external APIs, planning, autonomous Git behavior, or Phase 4.6 functionality.
